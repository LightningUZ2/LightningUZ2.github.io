...
function loadWidget(waifuPath, apiPath) {
	...
	function registerEventListener() {
		...
		console.log(re);
		re.toString = () => {
			showMessage("哈哈，你打开了控制台，果然是大佬啊！随便看吧！", 6000, 9);
			return "";
		};
		$(document).on("copy", () => {
		...
		$(document).on("visibilitychange", () => {
			if (!document.hidden) showMessage("终于回来了呀,快来陪我玩QωQ~", 6000, 9);
		});
	}
	...
	function loadRandModel() {
		...
		$.ajax({
			...
			success: function(result) {
				if (result.textures["id"] == 1 && (modelTexturesId == 1 || modelTexturesId == 0)) showMessage("嘤嘤嘤,没衣服了QaQ", 4000, 10);
				else showMessage("换好啦！我的新衣服好看嘛？", 4000, 10);
